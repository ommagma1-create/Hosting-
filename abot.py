    ...
    try:
        if int(hcfg("jail_runs") or 0):
            ...
            try:
                ...
                pass
            env.update(guard_env(uid, fid, jail, extra_write=[cwd]))
...
def BTN(text, data):
    """Callback button."""
    return types.InlineKeyboardButton(str(text), callback_data=str(data)[:63])
...
def on_text(message):
    ...
    if not text:
        return
    if text.startswith("/"):
        send(uid, _e("info") + " Unknown command. Use /help for the full list.", kb_main(uid))
        return
...
def set_user_font(uid, style):
    """Store the chosen style on the member record."""
...
extension_menu_button(_e("edit") + " Fonts", "font_panel", row=2)
...
HARDEN_DEFAULTS = {
    ...
    "block_subprocess": 1,   # deny shells / exec / spawn inside user code
    "block_ctypes": 1,       # deny ctypes + native library loading
...
def register_panel_menu():
    ...
    buttons = [
        ...
        (_e("edit") + " Fonts", "font_panel", False, 2),
