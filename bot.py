...
# Largest points/XP/coin change accepted anywhere. SQLite integers stop at
# 2**63-1, so a member typing a 22-digit number must never reach the driver.
AMOUNT_LIMIT = 1000000000


def clamp_amount(value, limit=AMOUNT_LIMIT):
    """Coerce any user supplied amount into a safe whole number."""
    try:
...
def add_points_admin(uid, pts, admin_uid=0):
    ...
    if abs(pts) > AMOUNT_LIMIT:
        return False, ("Keep the amount within "
                       + "{:,}".format(AMOUNT_LIMIT) + " points.")
    if not db_one("SELECT uid FROM users WHERE uid=?", (uid,)):
        return False, "That user has never used this bot."
...
def unshare_available():
    """Return the unshare binary only when the kernel really allows it.

    Railway containers and Android/Termux both ship the binary while
    denying user namespaces, so the old shutil.which() check produced
    "unshare failed: Permission denied" and a dead sandbox run. The probe
    ...
    try:
        ...
        _UNSHARE_OK = path if probe.returncode == 0 else ""
    except Exception:
        _UNSHARE_OK = ""
    return _UNSHARE_OK
...
def _network_wrapper(command):
    ...
    os.environ["SIGMA_NETNS"] = "1" if unshare_available() else "0"
    unshare = unshare_available()
    if not unshare:
        log.info("Netns isolation unavailable; guard allowlist in force")
        return list(command), False
...
def sandbox_interpreter(uid, command):
    """Swap the system interpreter for the owner's venv when one exists.

    Sandbox reviews used to launch the bare system python3, so a file whose
    requirements had already been installed into the uploader venv still
    died with ModuleNotFoundError. Reviewing with the same interpreter the
    file will really run under also makes the verdict honest.
    """
    cmd = list(command)
    if not cmd:
        return cmd
    ...
    try:
        ...
        log.debug("venv lookup failed: %s", exc)
        return cmd
    if py and Path(str(py)).exists():
        cmd[0] = str(py)
    return cmd
...
def run_in_sandbox(uid, fid, timeout=None):
    ...
    command = sandbox_interpreter(uid, command)
...
def extension_callback(prefix, admin_only=False, exact=False):
    ...
    def decorator(func):
        ...
        identity = (str(prefix), bool(exact),
                    getattr(func, "__module__", "core"),
                    getattr(func, "__qualname__", getattr(func, "__name__", "")))
        with _ext_lock:
            for index, item in enumerate(EXTENSION_REGISTRY["callbacks"]):
                other = item.get("handler")
                if identity == (str(item.get("prefix")), bool(item.get("exact")),
                                getattr(other, "__module__", "core"),
                                getattr(other, "__qualname__",
                                        getattr(other, "__name__", ""))):
                    EXTENSION_REGISTRY["callbacks"][index] = entry
                    return func
            EXTENSION_REGISTRY["callbacks"].append(entry)
        return func
...
class _FsmMessage(object):
    """Minimal message stand-in for handlers written message-first.

    Some panels declare their input handler as (message, uid, state) while the
    text router calls (uid, state, text). This shim lets both shapes work, so
    a future module can be written either way without breaking replies.
    """
    ...
    def __init__(self, uid, text):
        ...
        self.entities = []
        self.chat = type("_FsmChat", (object,), {"id": int(uid), "type": "private"})()
        self.from_user = type(
            "_FsmUser", (object,),
            {"id": int(uid), "is_bot": False, "username": "", "first_name": ""},
        )()
...
def fsm_handler_shape(handler):
    """Return 'message_first' or 'uid_first' for an input handler."""
    key = getattr(handler, "__qualname__", None) or repr(handler)
    shape = _FSM_SHAPES.get(key)
    if shape:
        return shape
    shape = "uid_first"
    try:
        names = list(inspect.signature(handler).parameters)
        if names and str(names[0]).lower() in ("message", "msg"):
            shape = "message_first"
    except (TypeError, ValueError):
        shape = "uid_first"
    _FSM_SHAPES[key] = shape
    return shape


def call_fsm_handler(handler, uid, state, text):
    """Call an input handler using whichever argument order it declares."""
    if fsm_handler_shape(handler) == "message_first":
        return handler(_FsmMessage(uid, text), uid, state)
    return handler(uid, state, text)
...
def on_text(message):
    ...
    clear_state(uid)
    try:
        call_fsm_handler(handler, uid, state, text)
    except Exception as exc:
...
def _handle_cb(call, uid, cid, mid, data):
    ...
    if data.startswith("set_toggle_lang_"):
        ...
        edit(cid, mid, msg_settings(uid), kb_settings(uid))
        return
    if data in ("settings_upgrade", "menu_upgrade", "upgrade"):
        ack(call)
...
def set_user_font(uid, style):
    ...
    try:
        db_exec(
            ...
            " ON CONFLICT(uid, key) DO UPDATE SET value=excluded.value,"
            " updated_at=excluded.updated_at",
            (int(uid), "font", str(style), utcstamp()))
    except Exception as exc:
        log.debug("font mirror skipped: %s", exc)
    try:
...
def protected_paths():
    ...
    # guard_dir() is deliberately NOT here: CPython must read
    # sitecustomize.py from it to start at all, and the guard source is
    # shipped code, not a secret. Writes are still blocked because it
    # sits outside every jail write root.
    out = [str(DB_PATH), str(BASE_DIR / "backups")]
    try:
...
_SELF_DIR = str(os.environ.get("SIGMA_GUARD_SELF", "") or "")
_JAIL = str(os.environ.get("SIGMA_GUARD_JAIL", "") or "")
...
def _check_read(path):
    ...
    # The interpreter reads the guard itself before user code runs,
    # so this can never be a violation.
    if _SELF_DIR and _under(path, [_SELF_DIR]):
        return
    if low.endswith("sitecustomize.py"):
        return
...
def _user_frame():
    """True when a frame from the hosted file is on the stack.

    importlib, encodings and plenty of libraries call compile() while
    starting up. Only code living inside the jail counts as dynamic
    execution by the user.
    """
    roots = [r for r in ([_JAIL] + list(_WRITE_ROOTS)) if r]
    if not roots:
        return True
    try:
        depth = 2
        while depth < 40:
            ...
            if ("site-packages" not in low and "/lib/python" not in low
                    and "frozen importlib" not in low
                    and "sitecustomize" not in low
                    and _under(name, roots)):
                return True
            depth += 1
    except Exception:
        return False
...
def write_guard_module():
    ...
    try:
        ...
        if current.strip() != GUARD_SOURCE.strip():
            ...
            log.info("Guard module refreshed")
        try:
...
def guard_env(uid, fid, jail, net_mode=None, extra_write=()):
    ...
    return {
        ...
        "SIGMA_GUARD_SELF": str(guard_dir()),
        "SIGMA_GUARD_JAIL": str(jail),
    }
...
def harden_boot():
    ...
    try:
        ...
        log.error("Version 11.A layer failed: %s", exc)
    try:
...
# ---- s1 ----
# =============================================================================
# SECTION 31 - PROVENANCE, ORIGIN TRUST AND MANDATORY REVIEW
# =============================================================================
# This section answers one question for every file: where did this code come
# from, and who vouched for it?
#
# The threat it closes is the relay attack. A user registers a second hosting
# bot with SIGMA; that bot is harmless on its own so it passes review; later
# the user feeds malicious code to the second bot, which runs it inside our
# host without our scanner ever seeing it. Provenance means every file records
# how it arrived, and anything that did not come straight from a human in a
# private chat is forced through an admin decision.
# =============================================================================

ORIGIN_KINDS = {
    "direct": {"label": "Direct upload", "trust": 70, "emoji": "\U0001f464",
               "note": "Sent by the owner in a private chat with this bot."},
    "forward": {"label": "Forwarded message", "trust": 30, "emoji": "\U0001f4e4",
                "note": "Forwarded from another chat - the author is not the uploader."},
    "via_bot": {"label": "Relayed by another bot", "trust": 5, "emoji": "\U0001f916",
                "note": "Delivered through a third-party bot. Always needs review."},
    "channel": {"label": "Channel post", "trust": 20, "emoji": "\U0001f4e3",
                "note": "Came from a channel or group, not a person."},
    "editor": {"label": "In-bot editor", "trust": 60, "emoji": "\u270f\ufe0f",
               "note": "Written or patched with the built-in code editor."},
    "pull": {"label": "Pulled from a URL", "trust": 10, "emoji": "\U0001f517",
             "note": "Fetched from a remote address supplied by the uploader."},
    "restore": {"label": "Restored copy", "trust": 50, "emoji": "\u267b\ufe0f",
                "note": "Recovered from quarantine, backup or a previous version."},
    "unknown": {"label": "Unknown origin", "trust": 0, "emoji": "\u2753",
                "note": "No provenance recorded. Treat as untrusted."},
}

ORIGIN_ORDER = ["direct", "editor", "restore", "forward", "channel", "pull",
                "via_bot", "unknown"]

# Origins that can never skip human review, whatever AUTO_APPROVE says.
ORIGIN_FORCE_REVIEW = ("via_bot", "forward", "channel", "pull", "unknown")

TRUST_BANDS = (
    (80, "trusted", "Trusted", "\U0001f7e2"),
    (55, "normal", "Normal", "\U0001f535"),
    (30, "watch", "Watch list", "\U0001f7e1"),
    (1, "suspect", "Suspect", "\U0001f7e0"),
    (0, "hostile", "Hostile", "\U0001f534"),
)


def init_origin_tables():
    """Provenance, review decisions and per-user trust history."""
    statements = [
        """
        CREATE TABLE IF NOT EXISTS file_origin (
            ...
            forward_chat TEXT DEFAULT '',
            forward_user TEXT DEFAULT '',
        ...
        CREATE TABLE IF NOT EXISTS review_decisions (
            ...
            uid INTEGER NOT NULL DEFAULT 0,
            admin_uid INTEGER NOT NULL DEFAULT 0,
        ...
        """,
        "CREATE INDEX IF NOT EXISTS idx_origin_uid ON file_origin(uid)",
        "CREATE INDEX IF NOT EXISTS idx_origin_kind ON file_origin(kind)",
        "CREATE INDEX IF NOT EXISTS idx_review_fid ON review_decisions(fid)",
        "CREATE INDEX IF NOT EXISTS idx_relay_uid ON relay_events(uid)",
    ]
    for statement in statements:
        try:
            db_exec(statement)
        except Exception as exc:
            log.error("init_origin_tables failed: %s", exc)
    return True


def origin_meta(kind):
    """Description block for an origin kind, never None."""
    return ORIGIN_KINDS.get(str(kind or "unknown"), ORIGIN_KINDS["unknown"])


def trust_band(score):
    """Map a 0-100 trust score onto a band tuple."""
    try:
        value = int(score or 0)
    except (TypeError, ValueError):
        value = 0
    for floor, key, label, emoji in TRUST_BANDS:
        if value >= floor:
            return key, label, emoji
    return "hostile", "Hostile", "\U0001f534"


def describe_message_origin(message):
    """Work out how a Telegram message reached us.

    pyTelegramBotAPI exposes forward metadata inconsistently across versions,
    so every lookup here is defensive and the function never raises.
    """
    info = {"kind": "direct", "via_bot": "", "forward_chat": "",
            "forward_user": "", "source_url": "", "chat_type": "", "detail": ""}
    if message is None:
        info["kind"] = "unknown"
        return info
    chat = getattr(message, "chat", None)
    info["chat_type"] = str(getattr(chat, "type", "") or "")
    via = getattr(message, "via_bot", None)
    if via is not None:
        name = str(getattr(via, "username", "") or getattr(via, "id", "") or "bot")
        info["kind"] = "via_bot"
        info["via_bot"] = name
        info["detail"] = "Relayed by @" + name
        return info
    fwd_chat = getattr(message, "forward_from_chat", None)
    if fwd_chat is not None:
        title = str(getattr(fwd_chat, "title", "") or getattr(fwd_chat, "username", "") or "chat")
        kind = str(getattr(fwd_chat, "type", "") or "")
        info["kind"] = "channel" if kind in ("channel", "supergroup", "group") else "forward"
        info["forward_chat"] = title
        info["detail"] = "Forwarded from " + (kind or "chat") + " " + title
        return info
    fwd_from = getattr(message, "forward_from", None)
    if fwd_from is not None:
        author = str(getattr(fwd_from, "username", "") or getattr(fwd_from, "id", "") or "user")
        is_bot = bool(getattr(fwd_from, "is_bot", False))
        info["kind"] = "via_bot" if is_bot else "forward"
        info["forward_user"] = author
        if is_bot:
            info["via_bot"] = author
            info["detail"] = "Forwarded from bot @" + author
        else:
            info["detail"] = "Forwarded from user " + author
        return info
    if getattr(message, "forward_origin", None) is not None:
        info["kind"] = "forward"
        info["detail"] = "Forwarded message"
        return info
    if info["chat_type"] and info["chat_type"] != "private":
        info["kind"] = "channel"
        info["detail"] = "Posted in a " + info["chat_type"]
        return info
    author = getattr(message, "from_user", None)
    if author is not None and bool(getattr(author, "is_bot", False)):
        info["kind"] = "via_bot"
        info["via_bot"] = str(getattr(author, "username", "") or "bot")
        info["detail"] = "Sender is a bot account"
    return info


def get_user_trust(uid):
    """Current trust score for a user, seeded at 50 on first sight."""
    uid = int(uid or 0)
    row = db_one("SELECT score FROM user_trust WHERE uid=?", (uid,))
    if not row:
        try:
            db_exec(
                "INSERT INTO user_trust (uid, score, updated_at) VALUES (?,?,?)",
                (uid, 50, utcstamp()),
            )
        except Exception as exc:
            log.debug("user_trust seed failed: %s", exc)
        return 50
    try:
        return int(row.get("score") or 50)
    except (TypeError, ValueError):
        return 50


def adjust_user_trust(uid, delta, event=""):
    """Move a user's trust score, remember why, and return the new score."""
    uid = int(uid or 0)
    current = get_user_trust(uid)
    new = int(max(0, min(100, current + int(delta or 0))))
    label = str(event or "")[:120]
    counters = {"approvals": 0, "denials": 0, "violations": 0, "quarantines": 0}
    if label.startswith("approve"):
        counters["approvals"] = 1
    elif label.startswith("deny"):
        counters["denials"] = 1
    elif label.startswith("violation"):
        counters["violations"] = 1
    elif label.startswith("quarantine"):
        counters["quarantines"] = 1
    try:
        db_exec(
            "INSERT INTO user_trust (uid, score, approvals, denials, violations,"
            " quarantines, last_event, updated_at) VALUES (?,?,?,?,?,?,?,?)"
            " ON CONFLICT(uid) DO UPDATE SET score=excluded.score,"
            " approvals=user_trust.approvals+excluded.approvals,"
            " denials=user_trust.denials+excluded.denials,"
            " violations=user_trust.violations+excluded.violations,"
            " quarantines=user_trust.quarantines+excluded.quarantines,"
            " last_event=excluded.last_event, updated_at=excluded.updated_at",
            (uid, new, counters["approvals"], counters["denials"],
             counters["violations"], counters["quarantines"], label, utcstamp()),
        )
    except Exception as exc:
        log.error("adjust_user_trust failed: %s", exc)
    return new


def trust_row(uid):
    """Full trust record for a user with safe defaults."""
    row = db_one("SELECT * FROM user_trust WHERE uid=?", (int(uid or 0),))
    if row:
        return dict(row)
    return {"uid": int(uid or 0), "score": get_user_trust(uid), "approvals": 0,
            "denials": 0, "violations": 0, "quarantines": 0,
            "last_event": "", "updated_at": ""}


def record_origin(fid, uid, info, kind_override=""):
    """Persist provenance for a file and return the stored row."""
    fid = int(fid or 0)
    uid = int(uid or 0)
    data = dict(info or {})
    kind = str(kind_override or data.get("kind") or "unknown")
    meta = origin_meta(kind)
    base_trust = int(meta.get("trust") or 0)
    owner_trust = get_user_trust(uid)
    trust = int(max(0, min(100, (base_trust * 2 + owner_trust) // 3)))
    forced = 1 if kind in ORIGIN_FORCE_REVIEW else 0
    try:
        db_exec(
            "INSERT INTO file_origin (fid, uid, kind, via_bot, forward_chat,"
            " forward_user, source_url, chat_type, trust, forced_review, detail,"
            " created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)"
            " ON CONFLICT(fid) DO UPDATE SET kind=excluded.kind,"
            " via_bot=excluded.via_bot, forward_chat=excluded.forward_chat,"
            " forward_user=excluded.forward_user, source_url=excluded.source_url,"
            " chat_type=excluded.chat_type, trust=excluded.trust,"
            " forced_review=excluded.forced_review, detail=excluded.detail",
            (fid, uid, kind,
             str(data.get("via_bot") or "")[:80],
             str(data.get("forward_chat") or "")[:120],
             str(data.get("forward_user") or "")[:80],
             str(data.get("source_url") or "")[:300],
             str(data.get("chat_type") or "")[:24],
             trust, forced,
             str(data.get("detail") or meta.get("note") or "")[:300],
             utcstamp()),
        )
    except Exception as exc:
        log.error("record_origin failed: %s", exc)
    if kind == "via_bot":
        try:
            db_exec(
                "INSERT INTO relay_events (uid, fid, bot_name, kind, detail,"
                " created_at) VALUES (?,?,?,?,?,?)",
                (uid, fid, str(data.get("via_bot") or "")[:80], kind,
                 str(data.get("detail") or "")[:300], utcstamp()),
            )
        except Exception as exc:
            log.error("relay_events insert failed: %s", exc)
    return get_origin(fid)


def get_origin(fid):
    """Provenance row for a file, or a synthetic untrusted row."""
    row = db_one("SELECT * FROM file_origin WHERE fid=?", (int(fid or 0),))
    if row:
        return dict(row)
    meta = origin_meta("unknown")
    return {"fid": int(fid or 0), "uid": 0, "kind": "unknown", "via_bot": "",
            "forward_chat": "", "forward_user": "", "source_url": "",
            "chat_type": "", "trust": int(meta.get("trust") or 0),
            "forced_review": 1, "detail": str(meta.get("note") or ""),
            "created_at": ""}


def origin_requires_review(fid):
    """True when provenance alone forces a human decision."""
    row = get_origin(fid)
    if int(row.get("forced_review") or 0):
        return True
    return str(row.get("kind") or "unknown") in ORIGIN_FORCE_REVIEW


def record_review_decision(fid, admin_uid, decision, reason="", ai_verdict="", score=0):
    """Log an approve/deny decision for the audit trail."""
    origin = get_origin(fid)
    row = get_file(fid) or {}
    owner = int(row.get("uid") or origin.get("uid") or 0)
    try:
        db_exec(
            "INSERT INTO review_decisions (fid, uid, admin_uid, decision, reason,"
            " origin, scan_score, ai_verdict, created_at) VALUES (?,?,?,?,?,?,?,?,?)",
            (int(fid or 0), owner, int(admin_uid or 0), str(decision)[:24],
             str(reason)[:400], str(origin.get("kind") or ""), int(score or 0),
             str(ai_verdict)[:200], utcstamp()),
        )
    except Exception as exc:
        log.error("record_review_decision failed: %s", exc)
    if str(decision) == "approve":
        adjust_user_trust(owner, 4, "approve:" + str(fid))
    elif str(decision) == "deny":
        adjust_user_trust(owner, -12, "deny:" + str(fid))
    return True


def review_history(fid, limit=10):
    """Past decisions for one file."""
    return db_all(
        "SELECT * FROM review_decisions WHERE fid=? ORDER BY id DESC LIMIT ?",
        (int(fid or 0), int(limit)),
    )


def relay_history(uid, limit=10):
    """Relay deliveries recorded for one user."""
    return db_all(
        "SELECT * FROM relay_events WHERE uid=? ORDER BY id DESC LIMIT ?",
        (int(uid or 0), int(limit)),
    )


def pending_reviews(limit=25):
    """Files waiting for a human decision, riskiest provenance first."""
    rows = db_all(
        "SELECT f.id fid, f.uid uid, f.fname fname, f.fsize fsize,"
        " f.status status, f.uploaded uploaded FROM files f"
        " WHERE f.status IN ('pending','review') ORDER BY f.id DESC LIMIT ?",
        (int(limit),),
    )
    out = []
    for row in rows or []:
        item = dict(row)
        origin = get_origin(int(item.get("fid") or 0))
        item["origin"] = str(origin.get("kind") or "unknown")
        item["trust"] = int(origin.get("trust") or 0)
        out.append(item)
    out.sort(key=lambda entry: int(entry.get("trust") or 0))
    return out


# ---- s2 ----
# =============================================================================
# SECTION 31B - OFFLINE CODE SUMMARY AND BEHAVIOUR PROFILE
# =============================================================================
# The admin card must explain what a file does even when no AI provider is
# reachable. Everything here is pure standard library: the ast module walks
# Python sources, and regular expressions cover the other runners. The output
# is a dict the message builders can render directly.
# =============================================================================

BEHAVIOUR_GROUPS = {
    "network": {"label": "Network access", "emoji": "\U0001f310", "weight": 6},
    "filesystem": {"label": "File system writes", "emoji": "\U0001f4c1", "weight": 5},
    "process": {"label": "Process / shell control", "emoji": "\u2699\ufe0f", "weight": 9},
    "dynamic": {"label": "Dynamic code execution", "emoji": "\U0001f300", "weight": 10},
    "crypto": {"label": "Crypto / encoding", "emoji": "\U0001f510", "weight": 4},
    "persistence": {"label": "Persistence / autostart", "emoji": "\U0001f4cc", "weight": 9},
    "secrets": {"label": "Credential access", "emoji": "\U0001f511", "weight": 10},
    "telegram": {"label": "Telegram API use", "emoji": "\u2708\ufe0f", "weight": 3},
    "selfmod": {"label": "Self modification", "emoji": "\U0001f501", "weight": 8},
    "hosting": {"label": "Runs other people's code", "emoji": "\U0001f3e0", "weight": 10},
    "database": {"label": "Database access", "emoji": "\U0001f5c3\ufe0f", "weight": 4},
    "threads": {"label": "Threads / concurrency", "emoji": "\U0001f9f5", "weight": 2},
}

BEHAVIOUR_MODULES = {
    "socket": "network", "http": "network", "httpx": "network", "requests": "network",
    "urllib": "network", "urllib3": "network", "aiohttp": "network", "ftplib": "network",
    "smtplib": "network", "telnetlib": "network", "paramiko": "network",
    "websocket": "network", "websockets": "network", "curl_cffi": "network",
    "shutil": "filesystem", "pathlib": "filesystem", "glob": "filesystem",
    "tempfile": "filesystem", "zipfile": "filesystem", "tarfile": "filesystem",
    "subprocess": "process", "multiprocessing": "process", "pty": "process",
    "signal": "process", "ctypes": "process", "resource": "process", "psutil": "process",
    "marshal": "dynamic", "pickle": "dynamic", "dill": "dynamic", "importlib": "dynamic",
    "types": "dynamic", "py_compile": "dynamic", "codeop": "dynamic",
    "base64": "crypto", "binascii": "crypto", "hashlib": "crypto", "hmac": "crypto",
    "zlib": "crypto", "lzma": "crypto", "bz2": "crypto", "codecs": "crypto",
    "cryptography": "crypto", "Crypto": "crypto", "secrets": "crypto",
    "crontab": "persistence", "sched": "persistence", "atexit": "persistence",
    "keyring": "secrets", "dotenv": "secrets", "netrc": "secrets",
    "telebot": "telegram", "telethon": "telegram", "pyrogram": "telegram",
    "aiogram": "telegram", "tgcrypto": "telegram",
    "sqlite3": "database", "pymysql": "database", "psycopg2": "database",
    "pymongo": "database", "redis": "database", "sqlalchemy": "database",
    "threading": "threads", "asyncio": "threads", "concurrent": "threads",
    "os": "filesystem", "sys": "process", "platform": "process",
}

BEHAVIOUR_CALLS = {
    "eval": "dynamic", "exec": "dynamic", "compile": "dynamic",
    "__import__": "dynamic", "globals": "dynamic", "locals": "dynamic",
    "setattr": "dynamic", "getattr": "dynamic", "open": "filesystem",
    "input": "process",
}

BEHAVIOUR_ATTRS = {
    "system": "process", "popen": "process", "spawnl": "process", "execv": "process",
    "execve": "process", "fork": "process", "kill": "process", "remove": "filesystem",
    "unlink": "filesystem", "rmtree": "filesystem", "chmod": "filesystem",
    "rename": "filesystem", "makedirs": "filesystem", "write_text": "filesystem",
    "write_bytes": "filesystem", "b64decode": "crypto", "b64encode": "crypto",
    "decompress": "crypto", "loads": "dynamic", "getenv": "secrets",
    "environ": "secrets", "send_document": "telegram", "send_message": "telegram",
    "download_file": "telegram", "get_file": "telegram",
}

# Patterns that describe hosting behaviour: a file that itself accepts and runs
# other people's code. These are the relay bots we must never auto-approve.
HOSTING_SIGNALS = (
    ("accepts document uploads", r"content_types\s*=\s*\[?[\"']document"),
    ("downloads what users send", r"download_file\s*\("),
    ("launches uploaded files", r"(Popen|run|call|system)\s*\([^)]{0,80}(python|node|bash|sh)"),
    ("stores per-user folders", r"(files|uploads|scripts)[/\\\"']\s*\+?\s*str\("),
    ("execs text it received", r"(exec|eval)\s*\(\s*(msg|message|text|code|payload)"),
    ("is another bot token holder", r"(TeleBot|Client|Bot)\s*\(\s*[\"'\w_]{0,40}(TOKEN|token)"),
    ("advertises hosting words", r"(host(ing)?|deploy|runner)\s*(bot|panel|service)"),
)

SECRET_PATTERNS = (
    ("telegram_token", r"\b\d{6,12}:[A-Za-z0-9_-]{30,}\b", "Telegram bot token"),
    ("aws_key", r"\bAKIA[0-9A-Z]{16}\b", "AWS access key"),
    ("private_key", r"-----BEGIN [A-Z ]*PRIVATE KEY-----", "Private key block"),
    ("slack_token", r"\bxox[abprs]-[A-Za-z0-9-]{10,}\b", "Slack token"),
    ("github_pat", r"\bgh[pousr]_[A-Za-z0-9]{30,}\b", "GitHub token"),
    ("openai_key", r"\bsk-[A-Za-z0-9]{20,}\b", "OpenAI style key"),
    ("generic_secret", r"(?i)(api[_-]?key|secret|passwd|password)\s*=\s*[\"'][^\"']{8,}", "Hardcoded credential"),
)


def _ast_summary(source):
    """Walk a Python AST and collect structure plus behaviour hits."""
    out = {
        "parsed": False, "syntax_error": "", "imports": [], "functions": [],
        "classes": [], "groups": {}, "calls": [], "loc": 0, "docstring": "",
    }
    try:
        tree = ast.parse(source)
        out["parsed"] = True
    except SyntaxError as exc:
        out["syntax_error"] = str(exc.msg) + " (line " + str(exc.lineno or 0) + ")"
        return out
    except Exception as exc:
    ...
    try:
        ...
        out["docstring"] = " ".join(str(doc).split())[:300]
    except Exception:
        out["docstring"] = ""
    groups = {}

    def bump(group, detail):
        if not group:
            return
        bucket = groups.setdefault(group, [])
        if detail and detail not in bucket and len(bucket) < 12:
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                ...
                out["imports"].append(name)
                bump(BEHAVIOUR_MODULES.get(name.split(".")[0], ""), name)
        elif isinstance(node, ast.ImportFrom):
            ...
            out["functions"].append(str(node.name))
        elif isinstance(node, ast.ClassDef):
            out["classes"].append(str(node.name))
        elif isinstance(node, ast.Call):
            func = node.func
            if isinstance(func, ast.Name):
                fname = str(func.id)
                out["calls"].append(fname)
                bump(BEHAVIOUR_CALLS.get(fname, ""), fname + "()")
            elif isinstance(func, ast.Attribute):
                attr = str(func.attr)
                out["calls"].append(attr)
                bump(BEHAVIOUR_ATTRS.get(attr, ""), attr + "()")
    out["imports"] = sorted(set(out["imports"]))[:40]
    out["functions"] = out["functions"][:40]
    out["classes"] = out["classes"][:20]
    out["groups"] = groups
    out["loc"] = source.count("\n") + 1
    return out


def _regex_summary(source):
    """Behaviour groups for non-Python sources."""
    groups = {}
    table = (
        ("network", r"(?i)\b(fetch|axios|XMLHttpRequest|curl|wget|http\.get|net\.Socket)\b"),
        ("process", r"(?i)\b(child_process|exec|spawn|system|popen|ShellExecute)\b"),
        ("dynamic", r"(?i)\b(eval|new Function|Invoke-Expression|iex)\b"),
        ("filesystem", r"(?i)\b(fs\.|unlink|rmdir|writeFile|chmod|rm -rf)\b"),
        ("crypto", r"(?i)\b(base64|atob|btoa|FromBase64String|gzip)\b"),
        ("persistence", r"(?i)\b(crontab|systemd|rc\.local|autostart|registry run)\b"),
        ("secrets", r"(?i)\b(process\.env|getenv|\.env|id_rsa|authorized_keys)\b"),
        ("telegram", r"(?i)\b(api\.telegram\.org|sendMessage|bot\d+:)\b"),
    )
    for group, pattern in table:
        try:
        ...
        if hits:
            seen = []
            for hit in hits:
                text = hit if isinstance(hit, str) else " ".join([h for h in hit if h])
                text = str(text).strip()
...
def find_secrets(source):
    """Credentials embedded in a source file, values never echoed in full."""
    found = []
    for key, pattern, label in SECRET_PATTERNS:
        try:
        ...
        if match:
            ...
            found.append({"key": key, "label": label, "masked": masked})
    return found


def hosting_signals(source):
    """Signals that this file is itself a hosting/relay service."""
    hits = []
    for label, pattern in HOSTING_SIGNALS:
        try:
...
def code_summary(fid, max_chars=200000):
    """Full offline behaviour profile for a stored file.

    Returns a dict with structure, behaviour groups, secrets, hosting signals
    and a 0-100 risk figure. Safe to call on binaries and missing files.
    """
    row = get_file(fid) or {}
    fname = str(row.get("fname") or "")
    path = Path(str(row.get("fpath") or ""))
    out = {
        "fid": int(fid or 0), "fname": fname, "exists": path.exists(),
        "size": int(row.get("fsize") or 0), "language": file_ext(fname) or "?",
        "parsed": False, "syntax_error": "", "imports": [], "functions": [],
        "classes": [], "groups": {}, "secrets": [], "hosting": [],
        "risk": 0, "lines": 0, "docstring": "", "headline": "",
        "binary": False,
    }
    if not path.exists():
        quarantined = db_one(
            "SELECT stored_path FROM quarantine WHERE fid=?"
            " ORDER BY id DESC LIMIT 1", (int(fid or 0),))
        if quarantined and str(quarantined.get("stored_path") or ""):
            candidate = Path(str(quarantined.get("stored_path")))
            if candidate.exists():
                path = candidate
                out["exists"] = True
    if not out["exists"]:
        out["headline"] = "File is not on disk; nothing to analyse."
        return out
    try:
        source = read_source_for_scan(path)
    except Exception as exc:
        out["headline"] = "Could not read the file: " + str(exc)
        return out
    if not isinstance(source, str):
        source = str(source or "")
    if len(source) > int(max_chars):
        source = source[:int(max_chars)]
    printable = sum(1 for ch in source[:4000] if ch.isprintable() or ch in "\r\n\t")
    if source and printable / float(min(len(source), 4000)) < 0.75:
        out["binary"] = True
        out["headline"] = "Binary or packed content - static reading is limited."
    out["lines"] = source.count("\n") + 1 if source else 0
    if out["language"] == ".py" and not out["binary"]:
        parsed = _ast_summary(source)
        out["parsed"] = bool(parsed.get("parsed"))
        out["syntax_error"] = str(parsed.get("syntax_error") or "")
        out["imports"] = list(parsed.get("imports") or [])
        out["functions"] = list(parsed.get("functions") or [])
        out["classes"] = list(parsed.get("classes") or [])
        out["groups"] = dict(parsed.get("groups") or {})
        out["docstring"] = str(parsed.get("docstring") or "")
    else:
        out["groups"] = _regex_summary(source)
    out["secrets"] = find_secrets(source)
    out["hosting"] = hosting_signals(source)
    if out["hosting"]:
        out["groups"].setdefault("hosting", out["hosting"][:8])
    risk = 0
    for group in out["groups"]:
        risk += int(BEHAVIOUR_GROUPS.get(group, {}).get("weight", 2))
    risk += 12 * len(out["secrets"])
    if out["hosting"]:
        risk += 10 + 4 * len(out["hosting"])
    if out["binary"]:
        risk += 15
    if out["syntax_error"]:
        risk += 5
    out["risk"] = int(max(0, min(100, risk)))
    if not out["headline"]:
        out["headline"] = summary_headline(out)
    return out


def summary_headline(summary):
    """One plain sentence describing what the file appears to do."""
    data = dict(summary or {})
    groups = list((data.get("groups") or {}).keys())
    if data.get("hosting"):
        return ("Looks like another hosting or relay service: it can accept and"
                " run code supplied after review.")
    if "telegram" in groups and "network" in groups:
        return "A Telegram bot that talks to the API and makes outbound requests."
    if "dynamic" in groups and "process" in groups:
        return "Builds and executes code or shell commands at runtime."
    if "secrets" in groups:
        return "Reads environment variables or credential files."
    if "network" in groups:
        return "A networked script that fetches or sends data."
    if "filesystem" in groups:
        return "Works mainly with local files."
    if data.get("functions"):
        return ("A " + str(data.get("language") or "script") + " module with "
                + str(len(data.get("functions") or [])) + " functions and no"
                " obvious risky behaviour.")
    return "Small script with no notable behaviour detected."


# ---- s3 ----
# =============================================================================
# SECTION 31C - FREE LLM BEHAVIOUR REVIEW
# =============================================================================
# A second opinion on every file, from a language model, with no paid account
# required. Providers are tried in order and the first one holding an API key
# wins; if none is configured the offline summary from Section 31B is used and
# the card says so honestly.
#
# All of these have a genuinely free tier:
#   * OpenRouter free models (DeepSeek V3, Llama 3.3 70B, Qwen coder, Gemini
#     Flash experimental) - key from openrouter.ai
#   * Groq - very fast Llama 3.3 70B, key from console.groq.com
#   * Google AI Studio - Gemini 2.0 Flash, key from aistudio.google.com
#   * Cerebras - Llama 3.3 70B, key from cloud.cerebras.ai
#   * Together AI - free Llama 3.3 70B tier
#   * GitHub Models - uses an ordinary GitHub token
#   * Ollama - fully local, no key and no internet at all
# Every provider below speaks the OpenAI chat-completions shape except Gemini
# and Ollama, which get their own request builders.
# =============================================================================
...
LLM_PROVIDERS = [
    {
        ...
        "style": "openai",
        "host": "openrouter.ai",
        "note": "Best free code reasoning. Free models are rate limited per day.",
    },
    {
        "key": "groq",
        ...
        "alternates": ["llama-3.1-8b-instant", "qwen/qwen3-32b"],
        "style": "openai",
        "host": "api.groq.com",
        "note": "Fastest free option, generous limits.",
    },
    {
        "key": "gemini",
        ...
        "alternates": ["gemini-2.0-flash-lite", "gemini-1.5-flash"],
        "style": "gemini",
        "host": "generativelanguage.googleapis.com",
        "note": "Large context window, good for long files.",
    },
    {
        "key": "cerebras",
        ...
        "model": "llama-3.3-70b",
        "alternates": ["llama3.1-8b"],
        "style": "openai",
        "host": "api.cerebras.ai",
        "note": "Free developer tier, very low latency.",
    },
    {
        "key": "together",
        ...
        "alternates": ["meta-llama/Llama-Vision-Free"],
        "style": "openai",
        "host": "api.together.xyz",
        "note": "Free Llama endpoint, no card required.",
    },
    {
        "key": "github",
        "label": "GitHub Models",
        ...
        "model": "gpt-4o-mini",
        "alternates": ["Phi-3.5-mini-instruct", "Meta-Llama-3.1-70B-Instruct"],
        "style": "openai",
        "host": "models.inference.ai.azure.com",
        "note": "Works with any ordinary GitHub personal access token.",
    },
    {
        "key": "huggingface",
        ...
        "alternates": ["meta-llama/Llama-3.3-70B-Instruct"],
        "style": "openai",
        "host": "router.huggingface.co",
        "note": "Free monthly credits on a normal HF account.",
    },
    {
        "key": "ollama",
        ...
        "alternates": ["llama3.2:3b", "deepseek-coder:6.7b"],
        "style": "ollama",
        "host": "127.0.0.1",
        "note": "Runs on your own machine. Nothing leaves the server.",
    },
]
...
AI_SYSTEM_PROMPT = (
    "You are a code security reviewer for a script hosting service. You receive"
    " a source file that a stranger uploaded. Decide whether running it on a"
    " shared server is safe. Malicious means: stealing tokens, credentials or"
    " environment variables; reading or exfiltrating other users' files or the"
    " host database; deleting or encrypting data; reverse shells; self"
    " replication; downloading and executing further payloads; persistence or"
    " autostart; crypto mining; hidden or obfuscated logic; or acting as a"
    " secondary hosting service that would run unreviewed code later."
    " Answer with strict JSON only, no markdown fence, using exactly these"
    " keys: verdict (one of safe, suspicious, malicious), confidence (0-100),"
    " summary (two sentences describing what the code does), risks (array of"
    " short strings), capabilities (array of short strings), recommendation"
    " (one of approve, sandbox, deny)."
)


def init_ai_tables():
    """Cache for model verdicts so we never pay the latency twice."""
    statements = [
        """
        ...
        """,
        "CREATE INDEX IF NOT EXISTS idx_ai_fid ON ai_reviews(fid)",
        "CREATE INDEX IF NOT EXISTS idx_ai_checksum ON ai_reviews(checksum)",
    ]
    for statement in statements:
        try:
            db_exec(statement)
        except Exception as exc:
            log.error("init_ai_tables failed: %s", exc)
    return True


def acfg(key, default=None):
    """Read an AI setting, falling back to the shipped default."""
    if default is None:
        default = LLM_DEFAULTS.get(key, 0)
    try:
        raw = cfg_get("ai_" + str(key).replace("ai_", ""), None)
    except Exception:
        raw = None
    if raw is None or raw == "":
        return default
...
def acfg_set(key, value):
    """Persist an AI setting."""
    try:
        cfg_set("ai_" + str(key).replace("ai_", ""), str(value), 0)
        return True
    except Exception as exc:
        log.error("acfg_set failed: %s", exc)
        return False


def provider_meta(key):
    """Provider definition by key, or None."""
    for provider in LLM_PROVIDERS:
        if str(provider.get("key")) == str(key):
            return dict(provider)
    return None


def provider_key_value(provider):
    """The credential for a provider, from the environment or bot_config."""
    env_name = str(provider.get("env") or "")
    value = str(os.environ.get(env_name, "") or "").strip()
    if not value:
        try:
            value = str(cfg_get("aikey_" + str(provider.get("key")), "") or "").strip()
        except Exception:
            value = ""
    if not value and str(provider.get("key")) == "ollama":
        # Ollama needs no key; treat a reachable default host as configured.
        return "local"
    return value


def available_providers():
    """Providers that currently hold a usable credential."""
    out = []
    for provider in LLM_PROVIDERS:
        if provider_key_value(provider):
            out.append(dict(provider))
    return out


def select_provider():
    """Pick the provider to use for the next review."""
    wanted = str(acfg("provider", "auto") or "auto")
    if wanted and wanted != "auto":
        provider = provider_meta(wanted)
        if provider and provider_key_value(provider):
            return provider
    for provider in LLM_PROVIDERS:
        if provider_key_value(provider):
            return dict(provider)
...
def _http_json(url, payload, headers, timeout):
    """POST JSON and return the decoded response. Raises on transport error."""
...
def _extract_text(provider, data):
    """Pull the assistant message out of any provider response shape."""
    style = str(provider.get("style") or "openai")
    try:
        if style == "gemini":
        ...
        if choices:
            ...
            if isinstance(content, list):
                return "".join([str(chunk.get("text") or "") for chunk in content])
            return str(content or "")
    except Exception as exc:
        log.debug("_extract_text failed: %s", exc)
    return str((data or {}).get("_raw") or "")


def _parse_verdict(text):
    """Turn model output into a verdict dict, tolerating stray prose."""
    out = {"verdict": "", "confidence": 0, "summary": "", "risks": [],
    ...
    try:
        ...
        for word in ("malicious", "suspicious", "safe"):
            if word in lowered:
                out["verdict"] = word
                break
        out["summary"] = " ".join(str(text or "").split())[:600]
        return out
    if not isinstance(data, dict):
        return out
    verdict = str(data.get("verdict") or "").strip().lower()
    if verdict not in ("safe", "suspicious", "malicious"):
        verdict = "suspicious" if verdict else ""
    out["verdict"] = verdict
    try:
        out["confidence"] = int(max(0, min(100, int(float(data.get("confidence") or 0)))))
    except (TypeError, ValueError):
        out["confidence"] = 0
    out["summary"] = " ".join(str(data.get("summary") or "").split())[:900]
    for field in ("risks", "capabilities"):
        value = data.get(field)
        items = []
        if isinstance(value, list):
            for entry in value[:12]:
                text_entry = " ".join(str(entry).split())[:160]
                if text_entry:
                    items.append(text_entry)
        elif value:
            items.append(" ".join(str(value).split())[:160])
        out[field] = items
    recommendation = str(data.get("recommendation") or "").strip().lower()
    if recommendation not in ("approve", "sandbox", "deny"):
        recommendation = ""
    out["recommendation"] = recommendation
    return out


def _build_request(provider, model, prompt, timeout):
    """Return (url, payload, headers) for one provider style."""
    style = str(provider.get("style") or "openai")
    key = provider_key_value(provider)
    if style == "gemini":
        url = (str(provider.get("url")) + str(model) + ":generateContent?key="
    ...
    if str(provider.get("key")) == "openrouter":
    ...
    return str(provider.get("url")), payload, headers


def build_ai_prompt(fid, summary=None):
    """Compose the user prompt: provenance, offline findings and source."""
    data = dict(summary or code_summary(fid))
    origin = get_origin(fid)
    row = get_file(fid) or {}
    limit = int(acfg("max_chars", 14000) or 14000)
    path = Path(str(row.get("fpath") or ""))
    source = ""
    if path.exists():
        try:
            ...
            source = ""
            log.debug("prompt read failed: %s", exc)
    if len(source) > limit:
        head = source[: int(limit * 0.7)]
        tail = source[-int(limit * 0.25):]
        source = head + "\n\n... [middle truncated] ...\n\n" + tail
    groups = ", ".join(sorted((data.get("groups") or {}).keys())) or "none"
    secrets = ", ".join([item.get("label", "") for item in data.get("secrets") or []]) or "none"
    hosting = "; ".join(data.get("hosting") or []) or "none"
    lines = [
        "File name: " + str(row.get("fname") or "unknown"),
        "Language: " + str(data.get("language") or "?"),
        "Size: " + fmt_size(int(row.get("fsize") or 0)),
        "Lines: " + str(data.get("lines") or 0),
        "How it reached us: " + str(origin_meta(origin.get("kind")).get("label"))
        + " - " + str(origin.get("detail") or ""),
        "Uploader trust score: " + str(get_user_trust(int(row.get("uid") or 0))) + "/100",
        "Static behaviour groups: " + groups,
        "Credentials found in source: " + secrets,
        "Hosting/relay signals: " + hosting,
        "Imports: " + (", ".join(data.get("imports") or []) or "none"),
        "",
        "Review this source and reply with the JSON object described in your"
        " instructions:",
        "",
...
def cached_ai_review(fid, checksum=""):
    """Most recent cached verdict for this exact file content."""
    hours = int(acfg("cache_hours", 24) or 24)
    row = None
    if checksum:
        row = db_one(
            "SELECT * FROM ai_reviews WHERE fid=? AND checksum=? AND error=''"
            " ORDER BY id DESC LIMIT 1", (int(fid or 0), str(checksum)))
    if not row:
        row = db_one(
            "SELECT * FROM ai_reviews WHERE fid=? AND error='' ORDER BY id DESC"
            " LIMIT 1", (int(fid or 0),))
        if row and checksum and str(row.get("checksum") or "") != str(checksum):
            return None
    if not row:
        return None
    if hours > 0:
        try:
            stamp = datetime.strptime(str(row.get("created_at")), "%Y-%m-%d %H:%M:%S")
            if (utcnow() - stamp).total_seconds() > hours * 3600:
                return None
        except Exception:
            pass
    out = dict(row)
    for field in ("risks", "capabilities"):
        try:
            out[field] = json.loads(str(out.get(field) or "[]"))
        except Exception:
            out[field] = []
    out["cached"] = True
    return out
...
def store_ai_review(fid, uid, provider, model, verdict, checksum, raw, error=""):
    """Persist one model answer, successful or not."""
    data = dict(verdict or {})
    try:
        db_exec(
            "INSERT INTO ai_reviews (fid, uid, provider, model, verdict,"
            " confidence, summary, risks, capabilities, recommendation,"
            " checksum, raw, error, created_at)"
            " VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
            (int(fid or 0), int(uid or 0), str(provider)[:40], str(model)[:80],
             str(data.get("verdict") or "")[:20],
             int(data.get("confidence") or 0),
             str(data.get("summary") or "")[:900],
             json.dumps(list(data.get("risks") or []))[:1200],
             json.dumps(list(data.get("capabilities") or []))[:1200],
             str(data.get("recommendation") or "")[:20],
             str(checksum or "")[:64], str(raw or "")[:3000],
             str(error or "")[:300], utcstamp()),
        )
    except Exception as exc:
        log.error("store_ai_review failed: %s", exc)
    return True


def offline_verdict(summary):
    """Turn the offline profile into a verdict when no model is reachable.

    The bot must never present "no opinion" to an admin, so the static risk
    score becomes the verdict and the recommendation follows from it.
    """
    data = dict(summary or {})
    risk = int(data.get("risk") or 0)
    groups = dict(data.get("groups") or {})
    hosting = list(data.get("hosting") or [])
    secrets = list(data.get("secrets") or [])
    if risk >= 60 or hosting or ("selfmod" in groups and "network" in groups):
        verdict, recommendation, confidence = "malicious", "deny", 55
    elif risk >= 25 or secrets or "dynamic" in groups or "process" in groups:
        verdict, recommendation, confidence = "suspicious", "sandbox", 50
    else:
        verdict, recommendation, confidence = "safe", "approve", 45
    risks = []
    for item in hosting[:4]:
        risks.append(str(item))
    for item in secrets[:3]:
        risks.append("credential in source: " + str(item.get("label")))
    for key in ("network", "process", "dynamic", "persistence", "selfmod"):
        if key in groups:
            info = BEHAVIOUR_GROUPS.get(key, {"label": key})
            risks.append(str(info.get("label")))
    return {
        "verdict": verdict,
        "recommendation": recommendation,
        "confidence": confidence,
        "summary": str(data.get("headline") or "Offline analysis only."),
        "risks": risks[:8],
        "capabilities": sorted(groups.keys()),
    }


def ai_review_file(fid, force=False):
    """Ask a free model to judge one file.

    Returns a dict that always contains verdict, summary, provider and error.
    Never raises: a missing provider, a rate limit or a network failure all
    degrade to the offline summary.
    """
    fid = int(fid or 0)
    row = get_file(fid) or {}
    uid = int(row.get("uid") or 0)
    summary = code_summary(fid)
    checksum = str(row.get("checksum") or "")
    if not int(acfg("review", 1) or 0):
        out = offline_verdict(summary)
        out.update({"ok": False, "provider": "offline",
                    "provider_label": "Offline analyser", "model": "static",
                    "error": "AI review is switched off - static analysis only.",
                    "offline": summary, "cached": False})
        return out
    if not force:
        cached = cached_ai_review(fid, checksum)
        if cached:
            cached["ok"] = True
            cached["offline"] = summary
            return cached
    ...
    if not provider:
        ...
        out.update({"ok": False, "provider": "offline",
                    "provider_label": "Offline analyser", "model": "static",
                    "error": "No AI provider configured yet - add a free key"
                              " with /aikey (OpenRouter, Groq, Gemini,"
                              " Cerebras, Together, GitHub Models,"
                              " HuggingFace) or run Ollama locally.",
                    "offline": summary, "cached": False})
        return out
    prompt = build_ai_prompt(fid, summary)
    timeout = int(acfg("timeout", 45) or 45)
    models = [str(acfg("model", "") or "") or str(provider.get("model"))]
    for alternate in provider.get("alternates") or []:
        if alternate not in models:
            models.append(alternate)
    last_error = ""
    for model in models[:3]:
        try:
            url, payload, headers = _build_request(provider, model, prompt, timeout)
            data = _http_json(url, payload, headers, timeout)
            text = _extract_text(provider, data)
            verdict = _parse_verdict(text)
            if not verdict.get("verdict") and not verdict.get("summary"):
                last_error = "Empty answer from " + str(model)
                continue
            store_ai_review(fid, uid, provider.get("key"), model, verdict,
                            checksum, text, "")
            out = dict(verdict)
            out["ok"] = True
            out["provider"] = str(provider.get("key"))
            out["provider_label"] = str(provider.get("label"))
            out["model"] = str(model)
            out["error"] = ""
            out["offline"] = summary
            out["cached"] = False
            log.info("AI review #%s via %s/%s -> %s", fid, provider.get("key"),
                     model, out.get("verdict"))
            return out
        except Exception as exc:
            last_error = type(exc).__name__ + ": " + str(exc)[:160]
            log.warning("AI review failed (%s/%s): %s", provider.get("key"),
                        model, last_error)
            continue
    store_ai_review(fid, uid, provider.get("key"), models[0], {}, checksum, "",
                    last_error)
    out = offline_verdict(summary)
    out.update({"ok": False, "provider": str(provider.get("key")),
                "provider_label": str(provider.get("label")) + " (offline fallback)",
                "model": models[0],
                "error": last_error or "The model could not be reached.",
                "offline": summary, "cached": False})
    return out


def ai_provider_status():
    """Rows describing every provider and whether it is ready."""
    out = []
    chosen = select_provider() or {}
    for provider in LLM_PROVIDERS:
        key_present = bool(provider_key_value(provider))
        out.append({
            "key": str(provider.get("key")),
            "label": str(provider.get("label")),
            "env": str(provider.get("env")),
            "model": str(provider.get("model")),
            "note": str(provider.get("note")),
            "ready": key_present,
            "active": str(provider.get("key")) == str(chosen.get("key") or ""),
        })
    return out


def ai_stats():
    """Counts for the AI panel."""
    total = int(db_val("SELECT COUNT(*) c FROM ai_reviews", (), 0) or 0)
    failed = int(db_val("SELECT COUNT(*) c FROM ai_reviews WHERE error<>''", (), 0) or 0)
    rows = db_all(
        "SELECT verdict, COUNT(*) c FROM ai_reviews WHERE error=''"
        " GROUP BY verdict ORDER BY c DESC") or []
    verdicts = {}
    for row in rows:
        verdicts[str(row.get("verdict") or "unknown")] = int(row.get("c") or 0)
    return {"total": total, "failed": failed, "verdicts": verdicts}


# ---- s4 ----
# =============================================================================
# SECTION 31D - REVIEW CARDS, SANDBOX REQUIREMENTS AND AI PANEL
# =============================================================================
# Every message in this section follows the house style: a header, a divider,
# labelled lines, and a keyboard where every decision is one tap. No admin ever
# has to type a command to approve or deny anything.
# =============================================================================
...
MISSING_MODULE_PATTERNS = (
    ...
    r"Error: Cannot find module ['\"]([A-Za-z0-9_.\-@/]+)['\"]",
)

VERDICT_CHIPS = {
    "safe": ("\U0001f7e2", "Safe"),
    "suspicious": ("\U0001f7e1", "Suspicious"),
    "malicious": ("\U0001f534", "Malicious"),
    "": ("\u26aa", "No verdict"),
}

RECOMMEND_CHIPS = {
    "approve": ("\u2705", "Approve"),
    "sandbox": ("\U0001f9ea", "Sandbox first"),
    "deny": ("\u26d4", "Deny"),
    "": ("\u2754", "Undecided"),
}
...
def remember_missing_modules(fid, modules):
    """Store modules a run needed so the install button knows what to fetch."""
    if not modules:
        return []
    try:
        cfg_set("missmod_" + str(int(fid or 0)), ",".join(modules), 0)
    except Exception as exc:
        log.debug("remember_missing_modules failed: %s", exc)
    return modules


def recalled_missing_modules(fid):
    """Modules remembered from the last failed run."""
    try:
        raw = str(cfg_get("missmod_" + str(int(fid or 0)), "") or "")
    except Exception:
        raw = ""
    return [part for part in raw.split(",") if part]


def sandbox_requirements(fid, output=""):
    """Combine static import detection with what the run actually complained about."""
    fid = int(fid or 0)
    from_run = parse_missing_modules(output)
    if from_run:
        remember_missing_modules(fid, from_run)
    else:
        from_run = recalled_missing_modules(fid)
    try:
        static = [dict(item) for item in (scan_requirements(fid, False) or [])]
    except Exception as exc:
        log.debug("scan_requirements failed: %s", exc)
        static = []
    names = []
    for item in static:
        name = str(item.get("module") or item.get("name") or "")
        state = str(item.get("state") or item.get("status") or "")
        if name and state != "installed" and name not in names:
            names.append(name)
    ...
    for name in names[:14]:
        try:
            package = pip_name_for(name)
        except Exception:
            package = name
        out.append({"module": name, "package": package,
                    "from_run": name in from_run})
    return out


def msg_sandbox_card(uid, fid, result):
    """Verdict card for admins after a sandbox run, requirements included."""
    row = get_file(fid) or {}
    origin = get_origin(fid)
    meta = origin_meta(origin.get("kind"))
    data = dict(result or {})
    output = str(data.get("output") or "")
    missing = sandbox_requirements(fid, output)
    band_key, band_label, band_emoji = trust_band(origin.get("trust"))
    lines = [
        _e("scan") + " " + B("SANDBOX VERDICT NEEDED"),
        divider(),
        _e("file") + " " + B("File: ") + C("#" + str(fid)) + " "
        + esc(str(row.get("fname") or "?")),
        _e("user") + " " + B("Owner: ") + C(str(row.get("uid") or uid)),
        str(meta.get("emoji")) + " " + B("Arrived as: ") + esc(str(meta.get("label"))),
        band_emoji + " " + B("Uploader trust: ") + C(str(origin.get("trust")) + "/100")
        + "  " + I(band_label),
        divider(),
        _e("ok") + " " + B("Exit code: ") + C(str(data.get("exit_code"))),
        _e("clock") + " " + B("Duration: ")
        + C(str(int(int(data.get("duration_ms") or 0) / 1000)) + "s"),
        _e("web") + " " + B("Network: ")
        + C("blocked" if int(data.get("network_blocked") or 0) else "allowed"),
        _e("key") + " " + B("Tokens neutralised: ") + C(str(data.get("replaced") or 0)),
    ]
    if missing:
        lines.append(divider())
        lines.append(_e("box") + " " + B("Requirements this file needs"))
        for item in missing[:8]:
            flag = _e("warn") if item.get("from_run") else _e("dot")
            lines.append("   " + flag + " " + C(str(item.get("module")))
                         + "  " + I("pip: " + str(item.get("package"))))
        lines.append(I("Install them below, then re-run the sandbox for a fair verdict."))
    if output:
        lines.append(divider())
        lines.append(_e("log") + " " + B("Captured output"))
        lines.append(PRE(trunc(output, 900)))
    return "\n".join(lines)


def kb_sandbox_card(fid, missing_count=0):
    """Decisions and requirement actions for a sandbox verdict."""
    fid = int(fid or 0)
    rows = [
        [BTN(_e("ok") + " Approve", "rvw_ok:" + str(fid)),
         BTN(_e("cross") + " Deny", "rvw_no:" + str(fid))],
        [BTN(_e("lock") + " Quarantine", "rvw_q:" + str(fid)),
         BTN(_e("scan") + " Re-run sandbox", "rvw_run:" + str(fid))],
    ]
    if int(missing_count or 0):
        rows.append([BTN(_e("box") + " Install " + str(int(missing_count))
                         + " requirement(s)", "sbreq:" + str(fid))])
    else:
        rows.append([BTN(_e("box") + " Requirements", "req:" + str(fid))])
    rows.append([BTN(_e("plus") + " Manual install", "reqman:" + str(fid)),
                 BTN(_e("db") + " Packages", "venvp")])
    rows.append([BTN(_e("robot") + " AI behaviour review", "aiask:" + str(fid)),
                 BTN(_e("list") + " Code summary", "aisum:" + str(fid))])
    rows.append([BTN(_e("info") + " Provenance", "orig:" + str(fid)),
                 BTN(_e("file") + " Open file", "file_" + str(fid))])
    rows.append([BACK("menu_admin")])
    return KB(*rows)


def msg_code_summary(fid):
    """Offline behaviour profile rendered for an admin."""
    data = code_summary(fid)
    row = get_file(fid) or {}
    lines = [
        _e("list") + " " + B("CODE SUMMARY"),
        divider(),
        _e("file") + " " + B("File: ") + C("#" + str(fid)) + " "
        + esc(str(data.get("fname") or "?")),
        _e("code") + " " + B("Language: ") + C(str(data.get("language") or "?"))
        + "   " + B("Lines: ") + C(str(data.get("lines") or 0))
        + "   " + B("Size: ") + C(fmt_size(int(row.get("fsize") or 0))),
        _e("target") + " " + B("Risk score: ") + C(str(data.get("risk")) + "/100"),
        divider(),
        _e("info") + " " + B("What it appears to do"),
        "   " + I(esc(str(data.get("headline") or "Unknown."))),
    ]
    if data.get("docstring"):
        lines.append("   " + I(esc(trunc(str(data.get("docstring")), 220))))
    if data.get("syntax_error"):
        lines.append(_e("warn") + " " + B("Syntax error: ")
                     + esc(str(data.get("syntax_error"))))
    groups = dict(data.get("groups") or {})
    if groups:
        lines.append(divider())
        lines.append(_e("scan") + " " + B("Capabilities detected"))
        for key in groups:
            info = BEHAVIOUR_GROUPS.get(key, {"label": key, "emoji": _e("dot")})
            detail = ", ".join([str(item) for item in (groups.get(key) or [])][:5])
            lines.append("   " + str(info.get("emoji")) + " "
                         + B(str(info.get("label"))) + "\n      "
                         + I(esc(trunc(detail, 150)) or "detected"))
    if data.get("secrets"):
        lines.append(divider())
        lines.append(_e("key") + " " + B("Credentials inside the source"))
        for item in data.get("secrets") or []:
            lines.append("   " + _e("warn") + " " + esc(str(item.get("label")))
                         + "  " + C(str(item.get("masked"))))
    if data.get("hosting"):
        lines.append(divider())
        lines.append(_e("warn") + " " + B("Hosting / relay signals"))
        for item in data.get("hosting") or []:
            lines.append("   " + _e("dot") + " " + esc(str(item)))
        lines.append(I("Files like this can run code that we never reviewed."
                       " They stay in strict mode permanently."))
    if data.get("imports"):
        lines.append(divider())
        lines.append(_e("box") + " " + B("Imports: ")
                     + C(trunc(", ".join(data.get("imports") or []), 300)))
    if data.get("functions"):
        lines.append(_e("code") + " " + B("Functions: ")
                     + C(trunc(", ".join(data.get("functions") or []), 300)))
    return "\n".join(lines)


def kb_code_summary(fid):
    """Actions beside a code summary."""
    fid = int(fid or 0)
    return KB(
        [BTN(_e("robot") + " Ask the AI reviewer", "aiask:" + str(fid))],
        [BTN(_e("ok") + " Approve", "rvw_ok:" + str(fid)),
         BTN(_e("cross") + " Deny", "rvw_no:" + str(fid))],
        [BTN(_e("scan") + " Sandbox it", "rvw_run:" + str(fid)),
         BTN(_e("lock") + " Quarantine", "rvw_q:" + str(fid))],
        [BTN(_e("info") + " Provenance", "orig:" + str(fid)),
         BTN(_e("file") + " Open file", "file_" + str(fid))],
        [BACK("menu_admin")],
    )


def msg_ai_review(fid, review):
    """Render one model verdict, or explain why there is none."""
    data = dict(review or {})
    row = get_file(fid) or {}
    verdict = str(data.get("verdict") or "")
    chip = VERDICT_CHIPS.get(verdict, VERDICT_CHIPS[""])
    rec = RECOMMEND_CHIPS.get(str(data.get("recommendation") or ""),
                              RECOMMEND_CHIPS[""])
    lines = [
        _e("robot") + " " + B("AI BEHAVIOUR REVIEW"),
        divider(),
        _e("file") + " " + B("File: ") + C("#" + str(fid)) + " "
        + esc(str(row.get("fname") or "?")),
        chip[0] + " " + B("Verdict: ") + B(chip[1])
        + "   " + I("confidence " + str(data.get("confidence") or 0) + "%"),
        rec[0] + " " + B("Recommends: ") + esc(rec[1]),
    ]
    if data.get("provider_label"):
        lines.append(_e("gear") + " " + B("Model: ")
                     + C(str(data.get("model") or ""))
                     + "  " + I(str(data.get("provider_label") or ""))
                     + ("  " + I("cached") if data.get("cached") else ""))
    lines.append(divider())
    lines.append(_e("info") + " " + B("Summary"))
    lines.append("   " + I(esc(str(data.get("summary") or "No summary produced."))))
    risks = list(data.get("risks") or [])
    if risks:
        lines.append(divider())
        lines.append(_e("warn") + " " + B("Risks flagged"))
        for item in risks[:8]:
            lines.append("   " + _e("dot") + " " + esc(str(item)))
    caps = list(data.get("capabilities") or [])
    if caps:
        lines.append(divider())
        lines.append(_e("scan") + " " + B("Capabilities"))
        lines.append("   " + I(esc(trunc(", ".join([str(c) for c in caps]), 300))))
    if data.get("error"):
        lines.append(divider())
        lines.append(_e("warn") + " " + B("Reviewer unavailable"))
        lines.append("   " + I(esc(trunc(str(data.get("error")), 220))))
        lines.append("   " + I("The offline analyser was used instead - see"
                               " Code summary for the full picture."))
    return "\n".join(lines)


def kb_ai_review(fid):
    """Decisions after an AI verdict."""
    fid = int(fid or 0)
    return KB(
        [BTN(_e("ok") + " Approve", "rvw_ok:" + str(fid)),
         BTN(_e("cross") + " Deny", "rvw_no:" + str(fid))],
        [BTN(_e("reload") + " Ask again", "aire:" + str(fid)),
         BTN(_e("list") + " Code summary", "aisum:" + str(fid))],
        [BTN(_e("scan") + " Sandbox it", "rvw_run:" + str(fid)),
         BTN(_e("lock") + " Quarantine", "rvw_q:" + str(fid))],
        [BTN(_e("gear") + " AI settings", "aipanel"), BACK("menu_admin")],
    )


def msg_origin_card(fid):
    """Provenance detail for one file."""
    origin = get_origin(fid)
    meta = origin_meta(origin.get("kind"))
    row = get_file(fid) or {}
    owner = int(row.get("uid") or origin.get("uid") or 0)
    trust = trust_row(owner)
    band_key, band_label, band_emoji = trust_band(trust.get("score"))
    lines = [
        _e("info") + " " + B("FILE PROVENANCE"),
        divider(),
        _e("file") + " " + B("File: ") + C("#" + str(fid)) + " "
        + esc(str(row.get("fname") or "?")),
        str(meta.get("emoji")) + " " + B("Origin: ") + B(str(meta.get("label"))),
        "   " + I(esc(str(origin.get("detail") or meta.get("note")))),
        _e("target") + " " + B("File trust: ") + C(str(origin.get("trust")) + "/100"),
        _e("shield") + " " + B("Forced review: ")
        + (_e("ok") + " yes" if int(origin.get("forced_review") or 0) else _e("cross") + " no"),
    ]
    if origin.get("via_bot"):
        lines.append(_e("robot") + " " + B("Relayed by: ") + C("@" + str(origin.get("via_bot"))))
    if origin.get("forward_chat"):
        lines.append(_e("web") + " " + B("From chat: ") + esc(str(origin.get("forward_chat"))))
    if origin.get("forward_user"):
        lines.append(_e("user") + " " + B("From user: ") + esc(str(origin.get("forward_user"))))
    if origin.get("source_url"):
        lines.append(_e("link") + " " + B("Source URL: ") + C(trunc(str(origin.get("source_url")), 80)))
    lines.extend([
        divider(),
        _e("user") + " " + B("UPLOADER RECORD"),
        "   " + band_emoji + " " + B("Trust: ") + C(str(trust.get("score")) + "/100")
        + "  " + I(band_label),
        "   " + _e("ok") + " Approvals: " + C(str(trust.get("approvals") or 0))
        + "   " + _e("cross") + " Denials: " + C(str(trust.get("denials") or 0)),
        "   " + _e("warn") + " Violations: " + C(str(trust.get("violations") or 0))
        + "   " + _e("lock") + " Quarantines: " + C(str(trust.get("quarantines") or 0)),
    ])
    history = review_history(fid, 5)
    if history:
        lines.append(divider())
        lines.append(_e("log") + " " + B("Past decisions"))
        for item in history:
            lines.append("   " + _e("dot") + " " + C(str(item.get("decision")))
                         + " by " + C(str(item.get("admin_uid")))
                         + "  " + I(str(item.get("created_at"))))
    relays = relay_history(owner, 5)
    if relays:
        lines.append(divider())
        lines.append(_e("robot") + " " + B("Relay deliveries by this user"))
        for item in relays:
            lines.append("   " + _e("dot") + " @" + esc(str(item.get("bot_name") or "?"))
                         + "  " + I(str(item.get("created_at"))))
    return "\n".join(lines)


def kb_origin_card(fid):
    """Actions on a provenance card."""
    fid = int(fid or 0)
    return KB(
        [BTN(_e("ok") + " Approve", "rvw_ok:" + str(fid)),
         BTN(_e("cross") + " Deny", "rvw_no:" + str(fid))],
        [BTN(_e("robot") + " AI review", "aiask:" + str(fid)),
         BTN(_e("list") + " Code summary", "aisum:" + str(fid))],
        [BTN(_e("scan") + " Sandbox it", "rvw_run:" + str(fid)),
         BTN(_e("shield") + " Policy", "fpol:" + str(fid))],
        [BACK("menu_admin")],
    )


def msg_review_queue():
    """Everything waiting for a human decision, riskiest first."""
    rows = pending_reviews(20)
    lines = [_e("scan") + " " + B("REVIEW QUEUE"), divider()]
    if not rows:
        lines.append(_e("ok") + " " + I("Nothing is waiting. Every upload has been decided."))
        return "\n".join(lines)
    lines.append(I("Lowest trust first. Tap a file to decide."))
    lines.append("")
    for item in rows:
        meta = origin_meta(item.get("origin"))
        band_key, band_label, band_emoji = trust_band(item.get("trust"))
        lines.append(
            band_emoji + " " + C("#" + str(item.get("fid"))) + " "
            + B(esc(trunc(str(item.get("fname") or "?"), 28))) + "\n"
            + "   " + str(meta.get("emoji")) + " " + I(str(meta.get("label")))
            + "  " + _e("user") + " " + C(str(item.get("uid")))
            + "  " + _e("target") + " " + C(str(item.get("trust")) + "/100")
        )
    return "\n".join(lines)


def kb_review_queue():
    """One button per waiting file."""
    rows = []
    for item in pending_reviews(10):
        meta = origin_meta(item.get("origin"))
        rows.append([BTN(str(meta.get("emoji")) + " #" + str(item.get("fid"))
                         + " " + trunc(str(item.get("fname") or "?"), 20),
                         "rvw:" + str(item.get("fid")))])
    rows.append([BTN(_e("reload") + " Refresh", "rvwq"),
                 BTN(_e("robot") + " AI settings", "aipanel")])
    rows.append([BACK("menu_admin")])
    return KB(*rows)


def msg_ai_panel():
    """Provider status and settings for the AI reviewer."""
    providers = ai_provider_status()
    stats = ai_stats()
    ready = [item for item in providers if item.get("ready")]
    lines = [
        _e("robot") + " " + B("AI REVIEWER"),
        divider(),
        _e("gear") + " " + B("Status: ")
        + (_e("ok") + " active" if (int(acfg("review", 1) or 0) and ready)
           else _e("warn") + " offline analyser only"),
        _e("target") + " " + B("Providers ready: ") + C(str(len(ready)) + "/"
                                                        + str(len(providers))),
        _e("log") + " " + B("Reviews stored: ") + C(str(stats.get("total")))
        + "   " + B("failed: ") + C(str(stats.get("failed"))),
    ]
    verdicts = dict(stats.get("verdicts") or {})
    if verdicts:
        parts = []
        for key in ("safe", "suspicious", "malicious"):
            if key in verdicts:
                chip = VERDICT_CHIPS.get(key, VERDICT_CHIPS[""])
                parts.append(chip[0] + " " + str(verdicts[key]))
        if parts:
            lines.append(_e("scan") + " " + B("Verdicts: ") + "  ".join(parts))
    lines.extend([divider(), _e("list") + " " + B("FREE PROVIDERS"), ""])
    for item in providers:
        mark = _e("ok") if item.get("ready") else _e("cross")
        active = "  " + B("< active") if item.get("active") else ""
        lines.append(
            mark + " " + B(esc(str(item.get("label")))) + active + "\n"
            + "   " + _e("key") + " " + C(str(item.get("env")))
            + "   " + _e("gear") + " " + C(trunc(str(item.get("model")), 36)) + "\n"
            + "   " + I(esc(str(item.get("note"))))
        )
    lines.extend([
        divider(),
        I("Set any one key as an environment variable and restart, or use"
          " /aikey <provider> <key> to store it in the database. Ollama needs"
          " no key at all."),
    ])
    return "\n".join(lines)
...
def kb_ai_panel():
    """Toggles and provider selection."""
    rows = [[BTN((_e("ok") if int(acfg("review", 1) or 0) else _e("cross"))
                 + " AI review", "aitog:review"),
             BTN((_e("ok") if int(acfg("auto_deny", 0) or 0) else _e("cross"))
                 + " Auto-deny malicious", "aitog:auto_deny")]]
    line = []
    for item in ai_provider_status():
        mark = _e("ok") if item.get("ready") else _e("dot")
        line.append(BTN(mark + " " + trunc(str(item.get("label")), 18),
                        "aiprov:" + str(item.get("key"))))
        if len(line) == 2:
            rows.append(line)
            line = []
    if line:
        rows.append(line)
    rows.append([BTN(_e("reload") + " Auto-select", "aiprov:auto"),
                 BTN(_e("scan") + " Review queue", "rvwq")])
    rows.append([BACK("hsec")])
    return KB(*rows)


# ---- s5 ----
# =============================================================================
# SECTION 31E - WIRING: COMMANDS, CALLBACKS, UPLOAD HOOK AND BOOT
# =============================================================================
# Nothing above this point changes behaviour on its own. This part attaches the
# provenance layer to the upload path, publishes the commands and buttons, and
# raises the runtime ceiling to 30 hours as requested.
# =============================================================================

_S31_READY = False

V11_TIER_TIMEOUTS = {
    "free": 108000,        # 30 hours
    "basic": 172800,       # 2 days
    "pro": 259200,         # 3 days
    "premium": 604800,     # 7 days
    "enterprise": 2592000, # 30 days
}


def apply_v11_timeouts():
    """Raise every tier to at least the 30 hour floor."""
    changed = []
    for key, seconds in V11_TIER_TIMEOUTS.items():
        current = int(TIER_TIMEOUTS.get(key, 0) or 0)
        if current < seconds:
            TIER_TIMEOUTS[key] = int(seconds)
            changed.append(key)
    return changed


def classify_upload(fid, uid, message, kind_override=""):
    """Record provenance for a new upload and decide whether it may auto-approve.

    Returns (origin_row, forced_review). When forced_review is true the file is
    pushed back to 'pending' even if AUTO_APPROVE is on, and the admins get a
    decision card with the AI verdict attached.
    """
    info = describe_message_origin(message)
    origin = record_origin(fid, uid, info, kind_override)
    forced = bool(int(origin.get("forced_review") or 0))
    if not forced:
        return origin, False
    row = get_file(fid) or {}
    if str(row.get("status") or "") == "approved":
        try:
            ...
            log.error("forced review status update failed: %s", exc)
    meta = origin_meta(origin.get("kind"))
    try:
        set_run_mode(int(fid), "strict")
    except Exception as exc:
        log.debug("set_run_mode failed: %s", exc)
    log.info("Forced review for #%s (%s)", fid, origin.get("kind"))
    try:
        chlog("security", "Forced review: " + str(meta.get("label")),
              "File #" + str(fid) + " " + str(row.get("fname") or "")
              + " from " + str(origin.get("detail") or ""), uid)
    except Exception as exc:
        log.debug("chlog failed: %s", exc)
    notify_user(
        uid,
        _e("shield") + " " + B("Extra review needed") + "\n" + divider() + "\n"
        + str(meta.get("emoji")) + " " + B("Why: ") + esc(str(meta.get("note")))
        + "\n" + _e("file") + " " + B("File: ") + C("#" + str(fid)) + " "
        + esc(str(row.get("fname") or "")) + "\n\n"
        + I("Code that did not come straight from you in this chat is always"
            " checked by a human before it can run. You will get a message the"
            " moment a decision is made."),
    )
    threading.Thread(target=_review_card_async, args=(int(fid), int(uid)),
                     name="sigma-review-card", daemon=True).start()
    return origin, True


def _review_card_async(fid, uid):
    """Build the admin decision card off the upload thread."""
    try:
        summary = code_summary(fid)
        review = ai_review_file(fid)
        row = get_file(fid) or {}
        origin = get_origin(fid)
        meta = origin_meta(origin.get("kind"))
        ...
        chip = VERDICT_CHIPS.get(verdict, VERDICT_CHIPS[""])
        body = "\n".join([
            _e("shield") + " " + B("DECISION NEEDED - UNTRUSTED ORIGIN"),
            divider(),
            _e("file") + " " + B("File: ") + C("#" + str(fid)) + " "
            + esc(str(row.get("fname") or "?")),
            _e("user") + " " + B("Owner: ") + C(str(uid))
            + "   " + B("Trust: ") + C(str(get_user_trust(uid)) + "/100"),
            str(meta.get("emoji")) + " " + B("Arrived as: ")
            + B(str(meta.get("label"))),
            "   " + I(esc(str(origin.get("detail") or meta.get("note")))),
            divider(),
            chip[0] + " " + B("AI verdict: ") + B(chip[1])
            + "   " + I("confidence " + str(review.get("confidence") or 0) + "%"),
            _e("target") + " " + B("Static risk: ")
            + C(str(summary.get("risk")) + "/100"),
            _e("info") + " " + B("What it does"),
            "   " + I(esc(str(review.get("summary")
                              or summary.get("headline") or "Unknown."))),
        ])
        risks = list(review.get("risks") or []) or list(summary.get("hosting") or [])
        if risks:
            body += "\n" + divider() + "\n" + _e("warn") + " " + B("Risks")
            for item in risks[:6]:
                body += "\n   " + _e("dot") + " " + esc(str(item))
        if review.get("error"):
            body += ("\n" + divider() + "\n" + _e("warn") + " "
                     + I(esc(trunc(str(review.get("error")), 180))))
        notify_admins(body, kb_sandbox_card(fid, len(sandbox_requirements(fid))))
        if (int(acfg("auto_deny", 0) or 0) and verdict == "malicious"
                and int(review.get("confidence") or 0) >= 80):
            quarantine_file(fid, "AI verdict: malicious")
            record_review_decision(fid, 0, "deny", "auto-deny on AI verdict",
                                   verdict, int(summary.get("risk") or 0))
            notify_admins(_e("lock") + " " + B("Auto-denied ") + C("#" + str(fid))
                          + " " + I("AI reported malicious with high confidence."))
    except Exception as exc:
        log.error("_review_card_async failed: %s", exc)


def install_missing_for_file(fid, uid=0):
    """Install everything a file still needs. Returns (ok, message)."""
    fid = int(fid or 0)
    row = get_file(fid) or {}
    owner = int(row.get("uid") or uid or 0)
    wanted = sandbox_requirements(fid)
    if not wanted:
        return True, "Nothing to install - every import is already available."
    packages = []
    for item in wanted:
        package = str(item.get("package") or item.get("module") or "")
        if package and package not in packages:
            packages.append(package)
    ok, message, output = pip_install(owner, packages)
    if ok:
        try:
            cfg_set("missmod_" + str(fid), "", 0)
        except Exception as exc:
            log.debug("clear missmod failed: %s", exc)
    return ok, message


def register_s31_ui():
    """Commands, callbacks and buttons for the provenance and AI layer."""

    @extension_command("review", "Decision card for a file: /review <id>", True)
    def _cmd_review(message, uid):
        args = cmd_args(message).split()
        if not args or not args[0].isdigit():
            send(uid, msg_review_queue(), kb_review_queue())
            return
        fid = int(args[0])
        send(uid, msg_code_summary(fid), kb_code_summary(fid))

    @extension_command("reviewq", "Files waiting for a decision", True)
    def _cmd_reviewq(message, uid):
        send(uid, msg_review_queue(), kb_review_queue())

    @extension_command("summary", "Offline code summary: /summary <id>", True)
    def _cmd_summary(message, uid):
        args = cmd_args(message).split()
        if not args or not args[0].isdigit():
            send(uid, _e("info") + " " + I("Usage: ") + C("/summary <file id>"))
            return
        fid = int(args[0])
        send(uid, msg_code_summary(fid), kb_code_summary(fid))

    @extension_command("aireview", "Ask the AI reviewer: /aireview <id>", True)
    def _cmd_aireview(message, uid):
        args = cmd_args(message).split()
        if not args or not args[0].isdigit():
            send(uid, msg_ai_panel(), kb_ai_panel())
            return
        fid = int(args[0])
        send(uid, _e("robot") + " " + I("Asking the reviewer about ")
             + C("#" + str(fid)) + I(" ..."))
        review = ai_review_file(fid, True)
        send(uid, msg_ai_review(fid, review), kb_ai_review(fid))

    @extension_command("aipanel", "AI reviewer status and providers", True)
    def _cmd_aipanel(message, uid):
        send(uid, msg_ai_panel(), kb_ai_panel())

    @extension_command("aikey", "Store a provider key: /aikey <provider> <key>", True)
    def _cmd_aikey(message, uid):
        args = cmd_args(message).split()
        if len(args) < 2:
            names = ", ".join([str(item.get("key")) for item in ai_provider_status()])
            send(uid, _e("info") + " " + I("Usage: ") + C("/aikey <provider> <key>")
                 + "\n" + I("Providers: ") + C(names), kb_ai_panel())
            return
        provider = str(args[0]).lower()
        if not provider_meta(provider):
            send(uid, _e("cross") + " " + I("Unknown provider: ") + C(provider),
                 kb_ai_panel())
            return
        cfg_set("aikey_" + provider, " ".join(args[1:]).strip(), uid)
        acfg_set("provider", provider)
        send(uid, _e("ok") + " " + B("Key stored for ") + C(provider) + "\n"
             + I("This provider is now the active reviewer."), kb_ai_panel())

    @extension_command("aimodel", "Override the model: /aimodel <name|auto>", True)
    def _cmd_aimodel(message, uid):
        args = cmd_args(message).split()
        value = args[0] if args else "auto"
        acfg_set("model", "" if value == "auto" else value)
        send(uid, _e("gear") + " " + B("Model: ") + C(value), kb_ai_panel())

    @extension_command("origin", "Where a file came from: /origin <id>", True)
    def _cmd_origin(message, uid):
        args = cmd_args(message).split()
        if not args or not args[0].isdigit():
            send(uid, _e("info") + " " + I("Usage: ") + C("/origin <file id>"))
            return
        fid = int(args[0])
        send(uid, msg_origin_card(fid), kb_origin_card(fid))

    @extension_command("trust", "Trust record: /trust <uid>", True)
    def _cmd_trust(message, uid):
        args = cmd_args(message).split()
        target = int(args[0]) if args and args[0].isdigit() else uid
        trust = trust_row(target)
        band_key, band_label, band_emoji = trust_band(trust.get("score"))
        send(uid, "\n".join([
            _e("target") + " " + B("TRUST RECORD"),
            divider(),
            _e("user") + " " + B("User: ") + C(str(target)),
            band_emoji + " " + B("Score: ") + C(str(trust.get("score")) + "/100")
            + "  " + I(band_label),
            _e("ok") + " " + B("Approvals: ") + C(str(trust.get("approvals") or 0)),
            _e("cross") + " " + B("Denials: ") + C(str(trust.get("denials") or 0)),
            _e("warn") + " " + B("Violations: ") + C(str(trust.get("violations") or 0)),
            _e("lock") + " " + B("Quarantines: ") + C(str(trust.get("quarantines") or 0)),
            _e("clock") + " " + B("Last event: ") + I(esc(str(trust.get("last_event") or "none"))),
        ]), KB([BTN(_e("scan") + " Review queue", "rvwq")], [BACK("menu_admin")]))

    @extension_command("relays", "Files delivered by other bots", True)
    def _cmd_relays(message, uid):
        rows = db_all("SELECT * FROM relay_events ORDER BY id DESC LIMIT 15") or []
        lines = [_e("robot") + " " + B("RELAY DELIVERIES"), divider()]
        if not rows:
            lines.append(_e("ok") + " " + I("No file has ever arrived through another bot."))
        for row in rows:
            lines.append(_e("dot") + " " + C("#" + str(row.get("fid")))
                         + " @" + esc(str(row.get("bot_name") or "?"))
                         + "  " + _e("user") + " " + C(str(row.get("uid")))
                         + "  " + I(str(row.get("created_at"))))
        send(uid, "\n".join(lines), KB([BTN(_e("scan") + " Review queue", "rvwq")],
                                       [BACK("menu_admin")]))

    @extension_callback("rvwq", True, True)
    def _cb_rvwq(call, uid, cid, mid, data):
        edit(cid, mid, msg_review_queue(), kb_review_queue())

    @extension_callback("aipanel", True, True)
    def _cb_aipanel(call, uid, cid, mid, data):
        edit(cid, mid, msg_ai_panel(), kb_ai_panel())

    @extension_callback("rvw:", True)
    def _cb_rvw(call, uid, cid, mid, data):
        fid = _tail_int(data, "rvw:")
        edit(cid, mid, msg_code_summary(fid), kb_code_summary(fid))

    @extension_callback("aisum:", True)
    def _cb_aisum(call, uid, cid, mid, data):
        fid = _tail_int(data, "aisum:")
        edit(cid, mid, msg_code_summary(fid), kb_code_summary(fid))

    @extension_callback("orig:", True)
    def _cb_orig(call, uid, cid, mid, data):
        fid = _tail_int(data, "orig:")
        edit(cid, mid, msg_origin_card(fid), kb_origin_card(fid))

    @extension_callback("aiask:", True)
    def _cb_aiask(call, uid, cid, mid, data):
        fid = _tail_int(data, "aiask:")
        ack(call, "Asking the reviewer...")
        review = ai_review_file(fid)
        edit(cid, mid, msg_ai_review(fid, review), kb_ai_review(fid))

    @extension_callback("aire:", True)
    def _cb_aire(call, uid, cid, mid, data):
        fid = _tail_int(data, "aire:")
        ack(call, "Fresh review...")
        review = ai_review_file(fid, True)
        edit(cid, mid, msg_ai_review(fid, review), kb_ai_review(fid))

    @extension_callback("aitog:", True)
    def _cb_aitog(call, uid, cid, mid, data):
        key = str(data).split(":", 1)[1] if ":" in str(data) else "review"
        current = int(acfg(key, LLM_DEFAULTS.get("ai_" + key, 0)) or 0)
        acfg_set(key, 0 if current else 1)
        edit(cid, mid, msg_ai_panel(), kb_ai_panel())

    @extension_callback("aiprov:", True)
    def _cb_aiprov(call, uid, cid, mid, data):
        value = str(data).split(":", 1)[1] if ":" in str(data) else "auto"
        acfg_set("provider", value)
        ack(call, "Provider: " + value)
        edit(cid, mid, msg_ai_panel(), kb_ai_panel())

    @extension_callback("sbreq:", True)
    def _cb_sbreq(call, uid, cid, mid, data):
        fid = _tail_int(data, "sbreq:")
        ack(call, "Installing...")
        ok, message = install_missing_for_file(fid, uid)
        edit(cid, mid,
             (_e("ok") if ok else _e("warn")) + " " + B("Requirement install")
             + "\n" + divider() + "\n" + _e("file") + " " + B("File: ")
             + C("#" + str(fid)) + "\n" + I(esc(trunc(str(message), 600)))
             + "\n\n" + I("Re-run the sandbox to confirm the file now starts."),
             kb_sandbox_card(fid, len(sandbox_requirements(fid))))

    @extension_callback("rvw_run:", True)
    def _cb_rvw_run(call, uid, cid, mid, data):
        fid = _tail_int(data, "rvw_run:")
        ack(call, "Sandboxing...")
        row = get_file(fid) or {}
        owner = int(row.get("uid") or uid)
        result = run_in_sandbox(owner, fid)
        if not result.get("ok"):
            edit(cid, mid, _e("warn") + " " + B("Sandbox failed") + "\n"
                 + I(esc(str(result.get("error") or "unknown error"))),
                 kb_sandbox_card(fid, len(sandbox_requirements(fid))))
            return
        missing = sandbox_requirements(fid, str(result.get("output") or ""))
        edit(cid, mid, msg_sandbox_card(uid, fid, result),
             kb_sandbox_card(fid, len(missing)))

    @extension_callback("rvw_ok:", True)
    def _cb_rvw_ok(call, uid, cid, mid, data):
        fid = _tail_int(data, "rvw_ok:")
        row = get_file(fid) or {}
        owner = int(row.get("uid") or 0)
        db_exec("UPDATE files SET status='approved' WHERE id=?", (int(fid),))
        summary = code_summary(fid)
        record_review_decision(fid, uid, "approve", "approved from review card",
                               "", int(summary.get("risk") or 0))
        ack(call, "Approved")
        edit(cid, mid, _e("ok") + " " + B("APPROVED") + "\n" + divider() + "\n"
             + _e("file") + " " + C("#" + str(fid)) + " "
             + esc(str(row.get("fname") or "")) + "\n"
             + _e("user") + " " + B("Owner notified: ") + C(str(owner)) + "\n\n"
             + I("The owner can now run this file. Provenance and the decision"
                 " are recorded in the audit trail."),
             KB([BTN(_e("scan") + " Review queue", "rvwq")], [BACK("menu_admin")]))
        notify_user(owner, _e("ok") + " " + B("Your file was approved") + "\n"
                    + _e("file") + " " + C("#" + str(fid)) + " "
                    + esc(str(row.get("fname") or "")) + "\n"
                    + I("You can start it from My Files."))
        chlog("security", "Approved #" + str(fid), "by admin " + str(uid), owner)

    @extension_callback("rvw_no:", True)
    def _cb_rvw_no(call, uid, cid, mid, data):
        fid = _tail_int(data, "rvw_no:")
        row = get_file(fid) or {}
        owner = int(row.get("uid") or 0)
        db_exec("UPDATE files SET status='denied' WHERE id=?", (int(fid),))
        summary = code_summary(fid)
        record_review_decision(fid, uid, "deny", "denied from review card", "",
                               int(summary.get("risk") or 0))
        ack(call, "Denied")
        edit(cid, mid, _e("cross") + " " + B("DENIED") + "\n" + divider() + "\n"
             + _e("file") + " " + C("#" + str(fid)) + " "
             + esc(str(row.get("fname") or "")) + "\n\n"
             + I("The file can never be started. The uploader's trust score"
                 " was reduced."),
             KB([BTN(_e("lock") + " Quarantine too", "rvw_q:" + str(fid))],
                [BTN(_e("scan") + " Review queue", "rvwq")], [BACK("menu_admin")]))
        notify_user(owner, _e("cross") + " " + B("Your file was not approved")
                    + "\n" + _e("file") + " " + C("#" + str(fid)) + " "
                    + esc(str(row.get("fname") or "")) + "\n"
                    + I("Contact support if you believe this was a mistake."))
        chlog("security", "Denied #" + str(fid), "by admin " + str(uid), owner)

    @extension_callback("rvw_q:", True)
    def _cb_rvw_q(call, uid, cid, mid, data):
        fid = _tail_int(data, "rvw_q:")
        row = get_file(fid) or {}
        owner = int(row.get("uid") or 0)
        ok = quarantine_file(fid, "Quarantined by admin " + str(uid))
        record_review_decision(fid, uid, "deny", "quarantined from review card")
        adjust_user_trust(owner, -20, "quarantine:" + str(fid))
        ack(call, "Quarantined")
        edit(cid, mid, _e("lock") + " " + B("QUARANTINED") + "\n" + divider()
             + "\n" + _e("file") + " " + C("#" + str(fid)) + " "
             + esc(str(row.get("fname") or "")) + "\n"
             + _e("ok") + " " + B("Moved to the vault: ")
             + ("yes" if ok else "already there") + "\n\n"
             + I("The file is out of the hosting tree and cannot be executed,"
                 " edited or downloaded by its owner."),
             KB([BTN(_e("scan") + " Review queue", "rvwq")], [BACK("menu_admin")]))
        chlog("security", "Quarantined #" + str(fid), "by admin " + str(uid), owner)

    extension_menu_button(_e("robot") + " AI Reviewer", "aipanel", True, 12)
    extension_menu_button(_e("scan") + " Review queue", "rvwq", True, 12)
    return True


def _wrap_store_upload():
    """Attach provenance classification to the upload path exactly once."""
    global _store_upload
    if getattr(_store_upload, "_sigma_provenance", False):
        return False
    original = _store_upload

    def wrapped(uid, file_id, raw_name, size, message):
        result = original(uid, file_id, raw_name, size, message)
        fid = 0
        try:
            if isinstance(result, dict):
                fid = int(result.get("fid") or result.get("id") or 0)
            elif isinstance(result, (list, tuple)):
                for item in result:
                    if isinstance(item, int) and item > 0:
                        fid = int(item)
                        break
                    if isinstance(item, dict) and (item.get("fid") or item.get("id")):
                        fid = int(item.get("fid") or item.get("id"))
                        break
            elif isinstance(result, int):
                fid = int(result)
        except Exception as exc:
            log.debug("upload result parse failed: %s", exc)
        if not fid:
            row = db_one(
                "SELECT id FROM files WHERE uid=? ORDER BY id DESC LIMIT 1",
                (int(uid),))
            fid = int((row or {}).get("id") or 0)
        if fid:
            try:
                classify_upload(fid, uid, message)
            except Exception as exc:
                log.error("classify_upload failed: %s", exc)
        return result

    wrapped._sigma_provenance = True
    _store_upload = wrapped
    return True


def _wrap_menu_rows():
    """Never show an admin-only extension button to an ordinary user."""
    global extension_menu_rows
    if getattr(extension_menu_rows, "_sigma_scoped", False):
        return False
    original = extension_menu_rows
    ...
    def wrapped(uid):
        ...
        if is_admin(uid):
            return rows
        admin_labels = set()
        for button in EXTENSION_REGISTRY.get("menu_buttons") or []:
            if isinstance(button, dict) and button.get("admin_only"):
                admin_labels.add(str(button.get("data") or ""))
        cleaned = []
        for row in rows or []:
            keep = []
            for button in row:
                target = str(getattr(button, "callback_data", "") or "")
                if target and target in admin_labels:
                    continue
                keep.append(button)
            if keep:
                cleaned.append(keep)
    ...
    wrapped._sigma_scoped = True
    extension_menu_rows = wrapped
    return True


def s31_boot():
    """Start the provenance, summary and AI review layer."""
    global _S31_READY
    if _S31_READY:
        return True
    steps = (
        ("origin tables", init_origin_tables),
        ("ai tables", init_ai_tables),
        ("timeout ceiling", apply_v11_timeouts),
        ("upload provenance", _wrap_store_upload),
        ("menu scoping", _wrap_menu_rows),
        ("review ui", register_s31_ui),
    )
    for label, step in steps:
        try:
            step()
            log.info("Security layer step ok: %s", label)
        except Exception as exc:
            log.error("Security layer step failed (%s): %s", label, exc)
    ready = [item for item in ai_provider_status() if item.get("ready")]
    log.info("AI reviewer: %s provider(s) ready, offline analyser always on",
             len(ready))
    log.info("Runtime ceiling: free=%sh enterprise=%sh",
             int(TIER_TIMEOUTS.get("free", 0) / 3600),
             int(TIER_TIMEOUTS.get("enterprise", 0) / 3600))
    _S31_READY = True
    log.info("Version 11.A security layer ready")
    return True
...
REQUIRED_COMMANDS = (
    "review", "reviewq", "summary", "aireview", "aipanel", "aikey",
    "aimodel", "origin", "trust", "relays",
