...
def build_command(row):
    ...
    if not path.exists():
        return False, "File is missing on disk.", False
    ext = file_ext(fname)
    if ext == ".java":
...
def _sandbox_preexec():
    ...
    limits = [
        ...
        ("RLIMIT_NPROC", int(SANDBOX_LIMITS["processes"])),
        ("RLIMIT_CORE", 0),
    ]
...
def _network_wrapper(command):
    """Prefix a command with unshare to remove network access when possible."""
